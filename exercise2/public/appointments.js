...
firebase.auth().onAuthStateChanged(getCustAptList);

function getCustAptList(newUser) {
  user = newUser;
  if (user) {
...
document.getElementById('makeAppointment').addEventListener('click', addCustApt);

function addCustApt(ev) {
